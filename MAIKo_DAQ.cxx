...
static std::atomic<bool> end_flag{false};
...
int main(int argc, char *argv[]){
  ...
  thread Anode0  (GigaIwaki, "anode0",   ref(end_flag), ref(ready));
  usleep(50000);
  thread Anode1  (GigaIwaki, "anode1",   ref(end_flag), ref(ready));
  usleep(50000);  
  thread Anode2  (GigaIwaki, "anode2",   ref(end_flag), ref(ready));
  usleep(50000);  
  thread Anode3  (GigaIwaki, "anode3",   ref(end_flag), ref(ready));
  usleep(50000);  
  thread Anode4  (GigaIwaki, "anode4",   ref(end_flag), ref(ready));
  usleep(50000);  
  thread Anode5  (GigaIwaki, "anode5",   ref(end_flag), ref(ready));
  usleep(50000);  
  thread Cathode0(GigaIwaki, "cathode0", ref(end_flag), ref(ready));
  usleep(50000);  
  thread Cathode1(GigaIwaki, "cathode1", ref(end_flag), ref(ready));
  usleep(50000);  
  thread Cathode2(GigaIwaki, "cathode2", ref(end_flag), ref(ready));
  usleep(50000);  
  thread Cathode3(GigaIwaki, "cathode3", ref(end_flag), ref(ready));
  usleep(50000);  
  thread Cathode4(GigaIwaki, "cathode4", ref(end_flag), ref(ready));
  usleep(50000);  
  thread Cathode5(GigaIwaki, "cathode5", ref(end_flag), ref(ready));
  usleep(50000);  
  while (ready.load(std::memory_order_relaxed) < 12) usleep(1000);
  printf("All receiver threads ready: %d\n", ready.load(std::memory_order_relaxed));
  // reset counter
  ...
  printf("Press Ctrl-C to stop the DAQ.\n");

  while(true){
    sleep(1);
    if(end_flag.load(std::memory_order_relaxed)) break;
  }

  // Join
  Anode0.join();  Anode1.join();  Anode2.join();  Anode3.join();  Anode4.join();  Anode5.join();
  Cathode0.join(); Cathode1.join(); Cathode2.join(); Cathode3.join(); Cathode4.join(); Cathode5.join();
